...
class ExchangeEconomyClass:
    ...
    def plot_all(self):
        ...
        # e. 5a 
        ax_A.scatter(self.a1, self.a2 ,marker='s',color='gray',label='5a - Choice set restricted to C')

        # f. 5b
        ax_A.scatter(self.solve.x1,self.solve.x2,marker='s',color='yellow',label='5b - Unrestricted choice set')

        # g. 6a
        ax_A.scatter(self.solve_socialplanner.x1,self.solve_socialplanner.x2 ,marker='s',color='red',label='6a - Social planner solution')

        # Possible allocations
        ax_A.scatter(C[:, 0], C[:, 1], s=15, color='blue', alpha=0.2, label='Pareto allocations')
